...
for full_path in ${video_directory}/*.mp4; do
    ...
    -m ${model_directory} \
    -o ${project_dir}/proc/predicted_frames/1_subj/${base_name}.1_subj.predictions.slp
done

# Rendering
echo "rendering ${video_directory}"
for full_path in ${project_dir}/proc/predicted_frames/1_subj/*.predictions.slp; do
    echo ${full_path}
